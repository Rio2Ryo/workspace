...
## 2026-06-15
- **Shiro Loop 停止・停滞の再発防止:** Yakonより「やっぱ何も進んでいないように見える」と指摘。確認すると `shiro-loop-tick` / `shiro-loop-digest` cron が disabled で、tmux側も `permission-wait` / `blocked` を観測するだけで再投入できていなかった。両cronを enabled に戻し、`ops/shiro-loop-tick.mjs` の nudge 条件を修正して、`permission-wait` の `external-action` / `needs-yakon` / `done-signoff` と `blocked` を再投入対象にした。手動実行で13セッションへ再投入済み。今後は「cron disabled」「最終run超過」「permission-wait 3件以上で nudges=0」を検知したら、報告前に自己修復・再投入する。

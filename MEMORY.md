...
## 2026-06-15
- **Shiro Loop 停止・停滞の再発防止:** Yakonより「やっぱ何も進んでいないように見える」と指摘。確認すると `shiro-loop-tick` / `shiro-loop-digest` cron が disabled で、tmux側も `permission-wait` / `blocked` を観測するだけで再投入できていなかった。さらに両cronの `delivery.mode` が `none` で、動いても #白 に可視化されない設定だった。両cronを enabled に戻し、delivery を `announce` に変更。`ops/shiro-loop-tick.mjs` の nudge 条件を修正して、`permission-wait` の `external-action` / `needs-yakon` / `done-signoff` と `blocked` を再投入対象にした。手動実行で13セッションへ再投入済み。今後は「cron disabled」「最終run超過」「permission-wait 3件以上で nudges=0」「delivery none」を検知したら、報告前に自己修復・再投入する。
- **未実行予定を進捗として報告した問題:** 2026-06-15 07:47 JST、Yakonより「言っているだけで実行していない」と指摘。確認すると07:27の自動投稿は `nudges=0` / `acted=false` で、実行報告ではなく予定の羅列だった。`ops/shiro-loop-digest.mjs` を修正し、`last-report.json` の `acted=true` / `kind=nudged` のみを「実行したこと」と表示し、未実行は「未実行の整理待ち」と明示するよう変更。今後「やる」「次回から」「白がやること」は進捗扱いしない。進捗として報告してよいのは、実コマンド、検証結果、commit、push/deploy結果、明示的なtmux再投入など観測可能な実行だけ。
- **Discordスレッド一覧の誤報:** 2026-06-15 07:52 JST、Yakonより「スレッドある」と強く指摘。原因は親チャンネル限定 `thread-list` の空結果だけで「配下スレッド0件」と断定したこと。全guild active threadでは多数のスレッドが取得できた。今後Discordスレッド確認では、親チャンネル限定、全guild active threads、channel read/search の最低3経路を照合し、空結果を即断しない。
- **#白配下スレッド台帳化:** 2026-06-15 07:58 JST、Yakonより #白 配下スレッドの可視化と運用を改めて指摘。Discord RESTで active threads を取得し、`state/discord-thread-registry.json` と `state/shiro-channel-thread-registry.json` を作成。#白配下は22件、内訳は `[DONE]` 完了除外4件、tmuxあり要中身確認9件、tmuxなし停止/要確認9件。以後、#白配下の進行管理はこの台帳を起点に、スレッド内へ直接ステータス・次アクションを残す。
- **#白配下スレッドへの棚卸し投稿:** 2026-06-15 08:20 JST、Yakonの「お願いします」を受け、#白配下22件のうち `[DONE]` 4件を除外し、非DONE 18スレッドへ直接「現在状態 / 実行環境 / 次アクション」を投稿。以後、#白本体だけでなく、各スレッド内に進捗・停止理由・次アクションを残す。

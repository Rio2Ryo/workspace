...
function mutationRequiredFieldValidationError(action: 'create' | 'edit', item: Pick<FavoriteItem, 'tag' | 'name'>): string | null {
...
function importItemValidationError(value: unknown, index: number, source = 'API replace import'): string | null {
  ...
  if (parseImportRank(o.rank) === null) return detail('rank', '順位は1〜3で入力してください。')
...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (requiredFieldError) return send(res, 400, { error: requiredFieldError })

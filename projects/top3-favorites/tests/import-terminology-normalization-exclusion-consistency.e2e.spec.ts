...
import { uploadJsonImportFile, resetItemsByDelete, importPreviewImpactMath, importPreviewLive } from './e2e-helpers'
...
test('import preview uses consistent term "正規化除外" across live summary and impact math', async ({ page }) => {
  ...
  await expect(importPreviewLive(page)).toContainText('正規化除外1件')
})

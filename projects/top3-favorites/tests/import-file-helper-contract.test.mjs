...
test('[E2E-Helper][import-file] import validation errors expose structured repair details', async () => {
  ...
  assert.match(appSource, /検出した修正対象[\s\S]*visibleImportValidationIssues\.map/, 'App should show the full list of detected validation issues for multi-error recovery and reuse the filtered visible list when a field is selected')
  ...
  assert.match(appSource, /selectedImportValidationField[\s\S]*visibleImportValidationIssues[\s\S]*filter\(\(issue\) => issue\.field === selectedImportValidationField\)[\s\S]*aria-pressed=\{selectedImportValidationField === field\}[\s\S]*\{field\}の修正対象だけ表示/, 'App should let users filter the repair list from the field summary instead of forcing a full-list scan')
  ...
  assert.match(appSource, /copyImportValidationRepairs[\s\S]*issue\.row[\s\S]*issue\.path[\s\S]*issue\.field[\s\S]*issue\.fix[\s\S]*修正対象一覧をコピーしました。/, 'App should let users copy the full repair checklist for multi-error recovery')
  ...
  assert.match(validationSpec, /locator\('dt'\)\)\.toHaveText\(\['ファイル', '行', 'JSONパス', 'フィールド', '修正', '検出件数'\]\)/, 'E2E should verify the term labels including JSON path and total issue count, not just the joined alert text')
})

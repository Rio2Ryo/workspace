...
test('[E2E-Helper][import-file] import validation errors expose structured repair details', async () => {
  ...
  assert.match(appSource, /importValidationFieldCounts[\s\S]*reduce<Map<string, number>>[\s\S]*data-testid="import-validation-field-summary"[\s\S]*フィールド別内訳/, 'App should summarize multi-error validation issues by field so users can prioritize repeated fixes')
  ...
  assert.doesNotMatch(validationSpec, /getByRole\('button', \{ name: '(?:JSONパス一覧をコピー|修正対象一覧をコピー)' \}\)/, 'E2E specs should not duplicate import validation copy button accessible names')

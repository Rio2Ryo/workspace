...
test('[E2E-Helper][import-file] import validation errors expose structured repair details', async () => {
  ...
  assert.match(helperSource, /export function importValidationFieldSummaryItems/, 'tests/e2e-helpers.ts should export importValidationFieldSummaryItems(page)')
  assert.match(helperSource, /importValidationFieldSummaryItems[\s\S]*importValidationFieldSummary\(page\)\.locator\('li span'\)/, 'importValidationFieldSummaryItems helper should own the field-summary item selector')
  ...
  assert.match(validationSpec, /importValidationFieldSummaryItems\(page\)/, 'E2E should verify field-summary rows through the shared helper')
  ...
  assert.doesNotMatch(validationSpec, /importValidationFieldSummary\(page\)\.locator\('li span'\)|\.locator\('li span'\)/, 'E2E specs should not duplicate the import validation field-summary item selector')

...
- [ ] **正常入力（メモあり）**
  ...
  - 自動確認: `tests/create-persists-after-reload-and-api.e2e.spec.ts` がメモあり新規登録後の一覧表示・reload復元・API保存内容を検証する
...
- [ ] **必須入力不足**
  - 入力: タグまたは店舗名を空にして保存
  - 期待: 追加されず `タグと店舗名は必須です。` が表示され、エラーalertにフォーカスが移動する
  - 自動確認: `tests/error-clears-stale-success-notice.e2e.spec.ts` が必須入力不足時のエラー表示・alertフォーカス復帰・古い成功通知のクリアを検証する
...
- [ ] **JSONエクスポート自動保存ブロック**
  ...
  - 期待: アプリはクラッシュせず、`手動保存用JSON` のテキスト欄に現在DBと同じJSONが選択状態で表示される。`手動保存JSONをコピー` で同じJSONをクリップボードへコピーでき、推奨ファイル名を参考に `.json` として手動保存・再インポートできる。APIとブラウザ側スナップショットは変化しない
  - 自動確認: `tests/import-export-failure-browser-storage-api-drift-diagnostic.e2e.spec.ts` が自動保存ブロック時の手動保存用JSON fallback・フォーカス・コピー・再インポート可能性・API/ブラウザ側スナップショット不変を検証する
...
- [ ] **JSONインポート失敗（不正要素混在）**
  ...
  - 期待: 全体を反映しない（fail-closed）、既存データは保持され、エラーalertにフォーカスが移動する。さらにalert内の「修正したJSONを再選択」からそのまま修正版を選び直せ、コピー制限時は手動コピー用テキストを選択してコピーできる
  - 自動確認: `tests/import-fail-closed-matrix.e2e.spec.ts`、`tests/import-export-rank-validation.e2e.spec.ts`、`tests/import-validation-error-details.e2e.spec.ts` が不正要素・rank範囲外・重複ID/行番号付きエラー・エラーalertへのフォーカス復帰・修正版JSONの再選択導線・コピー制限時の手動コピーfallbackを検証する
...
- [ ] 同一JSON再インポート時に `差分なし（このインポートでデータ変更はありません）` が表示される
  - 自動確認: `tests/import-no-change-badge.e2e.spec.ts`、`tests/import-preview/live/import-preview-live-summary-concise.e2e.spec.ts` が差分なしバッジ・読み上げ要約を検証する
- [ ] 正規化除外がある時は `正規化除外予定の店舗: 店名（タグ名でTop3外: N位相当）` が表示される
  - 自動確認: `tests/import-excluded-reasons-preview.e2e.spec.ts`、`tests/import-preview/summary/import-preview-excluded-names-normalized-context.e2e.spec.ts`、`tests/import-preview/summary/import-preview-excluded-names-tag-context.e2e.spec.ts` が正規化除外予定の店舗名・インライン理由・正規化/タグ文脈の表示を検証する
